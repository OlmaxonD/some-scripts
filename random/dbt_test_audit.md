...
This post is about how to store dbt test results for auditing purposes.  
...
Note: For data platform, this post uses Snowflake as an example.

## Logs and Artifacts

By default, dbt will write logs to a  `logs/` directory, and all other artifacts (like run_results.json) to a `target/` directory.
...
- Maximum log file size of 10MB cannot be changed.
- The results of one `dbt test` command usually spans across multiple files.

### dbt `run_results.json` file

dbt stores the each command results in `run_results.json` file.

sample data generated when `dbt test` command is executed:
...
`results` array contains all the tests that run and their status as `pass`, `fail`, `warn` etc.
...
Even if the log files and run_results.json files can be stored using custom scripts such as renaming and/or moving files, it is not an ideal solution. 
Instead, more robust solution would be sotring the dbt test results in a database table. It allows easier analysis.

## Using dbt test --store-failures

When `--store-failures` option is enabled, dbt creates tables in a specific audit schema for every test. If test passes, the resulting table will be empty. If the test fails, the resulting table will contain failing records.
...
dbt uses jinja, jinja (hence dbt) supports macros. If you are not familiar with macros, they are similar to functions. With macro, we can write a dynamic sql statement using context variables dbt provides, and make dbt execute the sql statement.
...
- resource_type
- query_id: the query id generated by Snowflake when dbt ran the test
- name: test name
- status: whether test failed, passes, skipped etc.
- message: if the test failed, error message

First we need to create a table on our data platform:
